...
async function submitLead() {
  ...
  const { error } = await supabase.from('leads').insert([{ name, number, status: selectedStatus }]);
  if (error) return alert("Error adding lead: " + error.message);
...
async function loadLeads() {
  ...
  if (error) return alert("Error loading leads: " + error.message);
  leads = data;
  renderLeads();
}
